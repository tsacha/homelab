...
| ------ | ------------------- | ---------------------------------------------------------------------------------------- |
| `apps` | cluster-vars, infra | Gateway/gw-public + Gateway/gw-private (WG-only VIP), http-redirect, lldap, pocketid, vaultwarden |
...
| `gateway`  | cluster-vars, certs | GatewayClasses (cilium-gw-public/-private), Gateway/gw-private (all tenants), Gateway/gw-public (empty), wildcard Certificate, http-redirect |
...
- **wg lives on the VPS host, not in a pod** (`task gw:wg`, `tasks/wg-host.sh`): `wg0` has no
  `Endpoint` — the router (dynamic IP) initiates toward the VPS static IP and the host learns the
  peer address from the handshake. No SNAT, no DNS dependency, transposable as-is to Talos
  (`WireguardConfig` rejects hostname endpoints anyway). Cilium `devices` must include `wg0` so the
  gw-private VIPs are translated for traffic arriving through the tunnel.
...
"2006010215" }}`, byte-identical render within the hour, Reloader fires on the boundary). That
   restart used to take wireguard down with it — moot since the tunnel inversion (the router
   initiates, nothing resolves `wg6.home` anymore), but the serial behavior remains.
